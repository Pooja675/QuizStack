...
import PdfUpload from "./components/PdfUpload1";
import Quiz from "./components/Quiz1";

const GEMINI_API_KEY = "AIzaSyBAzj4vX9VrdptEqd-pomJXKzGmgLzKQXE";
...
- Response must be ONLY the JSON object, nothing else`;
      ...
      try {
        ...
        // Try to fix incomplete JSON by checking if questions array is incomplete
        try {
          // If the JSON ends mid-array, try to close it properly
          let fixedText = generatedText;
          
          // Check if we're missing closing brackets
      ...
      // If we got fewer questions than requested, that's okay - just use what we got
  ...
  const handleRegenerateQuiz = async () => {
    if (lastPdfFile) {
      setQuizData(null); // Clear current quiz
